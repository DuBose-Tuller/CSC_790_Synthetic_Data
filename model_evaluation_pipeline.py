...
# def _binary_fold_auc(est, X_tr, y_tr, X_te, y_te):
#     """Binary AUC with full safety; returns np.nan if undefined."""
#     # single-class test fold → undefined AUC
...
#             # Some estimators trained on a single class in the train split -> (n,1)
#             if proba.shape[1] == 1:
#                 return np.nan
#             p1 = proba[:, 1]
...
#             if dec.ndim > 1:
#                 dec = dec.ravel()
#             p1 = 1.0 / (1.0 + np.exp(-dec))  # sigmoid
#         else:
#             # Hard predictions fallback (not ideal for AUC but safe)
#             p1 = est.predict(X_te).astype(float)
#         return roc_auc_score(y_te, p1)
#     except Exception:
#         return np.nan
...
# def run_three_models_training(train_path, target_col, random_state=42):
#     """Train three models with KFold CV (binary target) - SILENT (manual CV)."""
#     # Load training data
#     train_data = pd.read_csv(train_path).replace('.', np.nan).dropna()
...
#     # Normal KFold CV (non-stratified)
...
#         fold_scores = []
#         # fresh estimator each fold to avoid state carryover
#         for tr_idx, te_idx in cv.split(X_train_proc, y_train_enc):
#             X_tr, X_te = X_train_proc.iloc[tr_idx], X_train_proc.iloc[te_idx]
#             y_tr, y_te = y_train_enc[tr_idx], y_train_enc[te_idx]
...
#             fold_scores.append(_binary_fold_auc(est, X_tr, y_tr, X_te, y_te))

#         cv_mean = float(np.nanmean(fold_scores)) if not np.all(np.isnan(fold_scores)) else np.nan
#         cv_std  = float(np.nanstd(fold_scores))  if not np.all(np.isnan(fold_scores)) else np.nan
